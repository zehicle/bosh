...
describe Bosh::OpenCrowbar::ExternalCpi do
  ...
  def self.it_calls_cpi_method(method, cpi_method, *arguments)
    ...
    before { stub_const('Bosh::OpenCrowbar::Config', config) }
    let(:config) { double('Bosh::OpenCrowbar::Config', logger: double(:logger, debug: nil), cpi_task_log: cpi_log_path) }
    ...
    context 'when response is not a valid JSON' do
      ...
      it 'raises an error' do
        expect { call_cpi_method }.to raise_error(Bosh::OpenCrowbar::ExternalCpi::InvalidResponse)
      end
    end
    ...
    context 'when cpi command is not executable' do
      ...
      it 'raises MessageHandlerError' do
        expect {
          ...
          Bosh::OpenCrowbar::ExternalCpi::NonExecutable,
    ...
    describe 'error response' do
      ...
      context 'when cpi returns CpiError error' do
        it_raises_an_error(Bosh::OpenCrowbar::CpiError)
      end

      context 'when cpi returns NotImplemented error' do
        it_raises_an_error(Bosh::OpenCrowbar::NotImplemented)
      end

      context 'when cpi returns NotSupported error' do
        it_raises_an_error(Bosh::OpenCrowbar::NotSupported)
      end

      context 'when cpi returns CloudError error' do
        it_raises_an_error(Bosh::OpenCrowbar::CloudError)
      end

      context 'when cpi returns VMNotFound error' do
        it_raises_an_error(Bosh::OpenCrowbar::VMNotFound)
      end

      context 'when cpi returns a NoDiskSpace error' do
        it_raises_an_error_with_ok_to_retry(Bosh::OpenCrowbar::NoDiskSpace)
      end

      context 'when cpi returns a DiskNotAttached error' do
        it_raises_an_error_with_ok_to_retry(Bosh::OpenCrowbar::DiskNotAttached)
      end

      context 'when cpi returns a DiskNotFound error' do
        it_raises_an_error_with_ok_to_retry(Bosh::OpenCrowbar::DiskNotFound)
      end

      context 'when cpi returns a VMCreationFailed error' do
        it_raises_an_error_with_ok_to_retry(Bosh::OpenCrowbar::VMCreationFailed)
      end
      ...
      context 'when cpi raises unrecognizable error' do
        ...
        it 'raises an error constructed from error response' do
          expect {
            ...
            Bosh::OpenCrowbar::ExternalCpi::UnknownError,
